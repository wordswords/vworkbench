...
cp -f /opt/vworkbench/config/dotfiles/.tmux.conf ~/.tmux.conf
cp -f /opt/vworkbench/config/dotfiles/.vimrc ~/.vimrc
cp -f /opt/vworkbench/config/dotfiles/.zshenv ~/.zshenv
cp -f /opt/vworkbench/config/dotfiles/.zshrc ~/.zshenv
report_done
...
cd ~/.dotfiles
rm -rf ./*.jar
/opt/vworkbench/bin/user-scripts/download-latest-texidote-jar.sh
cd -
report_done
## General Install
report_progress 'Running ctags'
...
cp -f /opt/vworkbench/config/dotfiles/.p10k.zsh ~/.p10k.zsh || true
report_done
...
/opt/vworkbench/bin/user-scripts/deploy-ycm.sh
report_done
...
rm ~/.gitconfig
cp /opt/vworkbench/config/dotfiles/.gitconfig ~/.gitconfig
set -x
git config --global user.email "${VIMZ_EMAIL}"
set +x
report_done
report_progress 'Installing and configuring Joplin CLI notetaking app'
/opt/vworkbench/bin/user-scripts/update-joplin-cli.sh
cp -f /opt/vworkbench/config/extconfig/joplin.config ~/.dotfiles/joplin.config
...
sudo tar xzf /opt/vworkbench/assets/gaiman-fortunes.tgz -C /user/share/games/fortunes/
report_done
report_progress 'Stop unwanted changes dirtying up the dotfiles commit tracking'
/opt/vworkbench/bin/user-scripts/clean-git-checkout.sh ~/.dotfiles/.vim/pack/plugins/start/ || true
...
cp -f /home/vworkbench/config/extconfig/development.yml ~/.config/tmuxinator/development.yml
report_done
## OS specific stuff
cur_os=$(get_os)
...
if [[ $cur_os == 'linux' ]] ; then
    ...
    python3 /opt/vworkbench/bin/user-scripts/sync-clipboards-ubuntu.py
...
/opt/vworkbench/bin/user-scripts/24-bit-color.sh
report_done
